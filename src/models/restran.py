...
from src.models.components import (
    ResNetFeatureExtractor, ResNetFeatureExtractor_mHC, 
    AttentionFusion, PositionalEncoding, STNBlock,
...
class ResTranOCR_mHC(nn.Module):
    """
    ResTranOCR with Manifold-Constrained Hyper-Connections (mHC) in ResNet backbone.
    
    Architecture:
        Input [B, 5, 3, H, W]
        └─ STN (optional)
        └─ ResNet34 with mHC  → [B*5, 512, 1, W']  ← enhanced with mHC
        └─ AttentionFusion    → [B,   512, 1, W']
        └─ PositionalEncoding
    ...
    Key innovation:
        - ResNet34 residual connections are enhanced with mHC mechanism
        - Multi-stream processing in CNN backbone enables diverse feature learning
        - Doubly stochastic matrices ensure stable residual flow
        - Adaptive gating automatically learns optimal feature combination
    
    Benefits for License Plate Recognition:
        - mHC multi-stream processing can specialize different streams for:
          * Different illumination conditions (day/night)
          * Different viewing angles 
          * Different weather conditions (rain/fog)
        - Improved feature diversity helps handle Scenario-A vs Scenario-B variations
        - Dynamic routing adapts to input characteristics automatically
    
    Args:
        num_classes:     Vocab size (chars + 1 blank for CTC).
        transformer_heads:   Number of attention heads.
        transformer_layers:  Number of transformer encoder layers.
        transformer_ff_dim:  FFN inner dimension.
        dropout:         Dropout rate.
        use_stn:         Whether to prepend the Spatial Transformer Network.
        mhc_n:           Number of streams for mHC (default: 4, recommended: 2-8).
    """
    def __init__(
        self,
        num_classes: int,
        ...
        # 2. Backbone: ResNet34 with mHC
        self.backbone = ResNetFeatureExtractor_mHC(pretrained=False, n=mhc_n)
        
        # 3. Attention Fusion (unchanged)
        self.fusion = AttentionFusion(channels=self.cnn_channels)
        
        # 4. Positional Encoding + Transformer (unchanged)
        self.pos_encoder = PositionalEncoding(d_model=self.cnn_channels, dropout=dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass with mHC-enhanced ResNet backbone.
        ...
        # Enhanced feature extraction with mHC
        features = self.backbone(x_aligned)  # [B*F, 512, 1, W']
        fused = self.fusion(features)        # [B,   512, 1, W']
