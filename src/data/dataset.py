...
class MultiFrameDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            config: Config object for accessing advanced settings.
        """
        self.mode = mode
        self.config = config
    ...
    def _index_samples(self, tracks: List[str]) -> None:
        """Index all samples from selected tracks (with caching to speed up reruns)."""
        # Build cache path based on data root + mode + number of tracks
        cache_key = f"{len(tracks)}tracks_{self.mode}"
        cache_dir = os.path.join("data", ".cache")
        cache_file = os.path.join(cache_dir, f"sample_index_{cache_key}.json")
        ...
        if os.path.exists(cache_file):
            try:
                ...
                print(f"  ✅ Loaded {len(self.samples)} samples from cache (instant!)")
                return
            except Exception as e:
                print(f"  ⚠️  Cache load failed: {e}. Re-indexing...")
        ...
        # Apply rare character oversampling if enabled (training mode only)
        if self.mode == 'train' and getattr(self.config, 'OVERSAMPLE_RARE_CHARS', False):
            original_count = len(self.samples)
            self.samples = self._oversample_rare_characters(self.samples)
            print(f"  📊 Rare character oversampling: {original_count} -> {len(self.samples)} samples (+{len(self.samples) - original_count})")
        ...
        try:
            ...
            print(f"  💾 Cached index to {cache_file} for next run")
        except Exception as e:
    ...
    def _oversample_rare_characters(self, samples: List[Dict]) -> List[Dict]:
        """
        Oversample tracks containing rare characters to balance frequency.
        
        Based on EDA: Letters K,L,M,N,O,Q,U,V,W have < 2000 occurrences (vs A:7500).
        Duplicating these samples 3x increases their training frequency → better learning.
        
        Args:
            samples: Original sample list
        
        Returns:
            Balanced sample list with rare character samples duplicated
        """
        from src.utils.character_balance import CHAR_FREQUENCIES_FALLBACK
        import random
        
        # Get threshold from config
        threshold = getattr(self.config, 'RARE_CHAR_THRESHOLD', 2000)
        
        # Identify rare characters (use fallback for oversampling decision)
        rare_chars = [char for char, freq in CHAR_FREQUENCIES_FALLBACK.items() if freq < threshold]
        
        if not rare_chars:
            return samples
        
        print(f"    Rare characters (< {threshold} occurrences): {', '.join(rare_chars)}")
        
        # Identify samples containing rare characters
        rare_samples = []
        ...
        print(f"    Found {len(rare_samples)} samples with rare chars ({len(rare_samples)*100//len(samples)}% of dataset)")
        
        # Duplicate rare samples 3x (total 4x frequency counting original)
        oversampled = samples + rare_samples * 3
        
        # Shuffle to distribute rare samples throughout training
        random.Random(self.config.SEED).shuffle(oversampled)
